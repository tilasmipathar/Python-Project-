...
def Run_Shop(screen,coin):
	...
	card_width,card_height=(Image.open("Card_Shop/0.png")).size
	card_type=[]
	card_def=[]
	card_attack=[]
	card_image=[]
	...
	for i in range(0,3):
		...
		card_image.append(pygame.image.load("Card_Shop/"+str(card_type[i])+".png"))
		cardX.append(width*(i+1)/4-card_width)
		cardY.append(height/2-card_height/2-(1-i)*128)
	
	heading=pygame.font.Font("Card_Shop/OldLondon.ttf",64)
	body=pygame.font.Font("Card_Shop/Seagram.ttf",24)
	...
	((Image.open("Card_Shop/back.png")).resize((int(width),int(height)))).save("Card_Shop/back_adjusted.png")
	background=pygame.image.load("Card_Shop/back_adjusted.png")
	...
	if(coin>0):
		while running:
			...
			if(choice!=0):
				cardY[choice-1]+=5+((16*(cardY[choice-1]-height/2)/(height/2)) if cardY[choice-1]>=height/2 else 0);
			...
			for i in range(0,3):
				...
				screen.blit(pygame.image.load("Card_Shop/info.png"),(cardX[i]+card_image[i].get_width(),cardY[i]))
				for j in body_render[i]:
					screen.blit(j,(cardX[i]+card_image[i].get_width()+10,cardY[i]+height_displacement+10))
					height_displacement+=48
			screen.blit(pygame.image.load("Card_Shop/info.png"),(0,0))
			screen.blit(body.render("Coins: "+str(coin),True,(0,0,0)),(10,10))	
	...
	return Card(card_type[choice-1],card_def[choice-1],card_attack[choice-1],pygame.image.load("Card_Shop/"+str(card_type[choice-1])+".png"))
